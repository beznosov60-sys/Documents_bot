...
async def send_ocr_result(message: Message, result_dict: dict[str, Any]) -> None:
    ...
    def _format(label: str, value: Any) -> str:
        if not value:
            return f"{label}: ❌ не найдено"
        if label == "Серия" and isinstance(value, str) and len(value) == 4:
    ...
    for label in ordered_labels:
        lines.append(_format(label, result_dict.get(label)))

    required_labels = ["ФИО", "Серия", "Номер", "Кем выдан", "Дата"]
    missing_fields = [
        label for label in required_labels if not result_dict.get(label)
    ]
    if missing_fields:
...
async def handle_passport_photo(message: Message, state: FSMContext) -> None:
    ...
    recognized_fields = [label for label, value in ocr_result.items() if value]
    required_fields = ["ФИО", "Серия", "Номер", "Кем выдан", "Дата"]
    missing_fields = [label for label in required_fields if not ocr_result.get(label)]
